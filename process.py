import sys

lines = sys.stdin.readlines()

init=0
while "on(" not in lines[init]:
    init+=1

atoms = lines[init].split(" ")

obstacles=[]

rangeX=[]
rangeY=[]


for line in atoms:
    if line[0:6] != "rangeX":
        continue
    line=line.replace("rangeX","")
    line=line.replace("(","")
    line=line.replace(")","")
    rangeX.append(int(line))

for line in atoms:
    if line[0:6] != "rangeY":
        continue
    line=line.replace("rangeY","")
    line=line.replace("(","")
    line=line.replace(")","")
    rangeY.append(int(line))


for line in atoms:
    if line[0:3] != "obs":
        continue
    line=line.replace("obstacle","")
    line=line.replace("(","")
    line=line.replace(")","")
    tup = line.split(",")
    obstacles.append([int(tup[0]),int(tup[1])])

on = []
for line in atoms:
    if line[0:3] != "on(":
        continue
    line=line.replace("on(","")
    line=line.replace(")","")
    tup = line.split(",")
    on.append([int(tup[0][1:])-1,int(tup[1]),int(tup[2]),int(tup[3])])

objectives=[]
for line in atoms:
    if line[0:4] != "goal":
        continue
    line=line.replace("goal","")
    line=line.replace("(","")
    line=line.replace(")","")
    tup = line.split(",")
    objectives.append([int(tup[0][1:])-1,int(tup[1]),int(tup[2])])

robots=[]
for t in on:
    if t[0] not in robots:
        robots.append(t[0])

times=[]
for t in on:
    if t[3] not in times:
        times.append(t[3])
times.sort()

on.sort(key=lambda x:x[3])
on.sort(key=lambda x:x[0])

pos={}

for r in robots:
    pos['rob_'+str(r)] = [];

for t in on:
    pos['rob_'+str(t[0])].append([t[1],t[2]])


print('var xrange;');
print('var yrange;');
print('xrange=',max(rangeX),';');
print('yrange=',max(rangeY),';');

print('var objectives;')
print('objectives=',objectives)
print('var obstacles;')
print('obstacles=',obstacles)

print('var pos=[];')
i=0
for r in robots:
    print('pos['+str(i)+']=',pos['rob_'+str(r)],';')
    i+=1

print('var events=[]')
for t in times:
    print('events['+str(t)+']=',[[r,x,y] for [r,x,y,time] in on if time==t])

